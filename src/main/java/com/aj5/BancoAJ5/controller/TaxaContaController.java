...
public class TaxaContaController {
	...
	@ApiOperation(value="Consulta todos TaxaContas do sistema", notes="Consulta Assíncrona.")
	@GetMapping
	public ResponseEntity<List<TaxaConta>> getAll(){
		return ResponseEntity.ok(this.service.findAll());
	}
	
	@ApiOperation(value="Consulta TaxaConta específico do sistema", notes="Consulta Assíncrona.")
	@GetMapping("/{id}")
	public ResponseEntity<Optional<TaxaConta>> getByID(@PathVariable("id") Long id){
		return ResponseEntity.ok(this.service.getById(id));
	}
	
	@ApiOperation(value="Cadastra TaxaConta no sistema", notes="Consulta Assíncrona.")
	@PostMapping
	public String save(@RequestBody TaxaConta TaxaConta) {
		return this.service.create(TaxaConta).toString();
	}
	
	@ApiOperation(value="Deleta TaxaConta específico do sistema", notes="Consulta Assíncrona.")
	@DeleteMapping("/{id}")
