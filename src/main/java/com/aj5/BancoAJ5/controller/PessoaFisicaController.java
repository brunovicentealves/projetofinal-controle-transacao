...
public class PessoaFisicaController {
	...
	@ApiOperation(value="Consulta todos PessoaFisicas do sistema", notes="Consulta Assíncrona.")
	@GetMapping
	public ResponseEntity<List<PessoaFisica>> getAll(){
		return ResponseEntity.ok(this.service.findAll());
	}
	
	@ApiOperation(value="Consulta PessoaFisica específico do sistema", notes="Consulta Assíncrona.")
	@GetMapping("/{id}")
	public ResponseEntity<Optional<PessoaFisica>> getByID(@PathVariable("id") Long id){
		return ResponseEntity.ok(this.service.getById(id));
	}
	
	@ApiOperation(value="Cadastra PessoaFisica no sistema", notes="Consulta Assíncrona.")
	@PostMapping
	public String save(@RequestBody PessoaFisica PessoaFisica) {
		return this.service.create(PessoaFisica).toString();
	}
	
	@ApiOperation(value="Deleta PessoaFisica específico do sistema", notes="Consulta Assíncrona.")
	@DeleteMapping("/{id}")
